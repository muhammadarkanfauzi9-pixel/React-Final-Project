import React, { useEffect, useState } from "react"; // React + hooks untuk state & efek
import { useSelector, useDispatch } from "react-redux"; // Redux hooks
import { Link } from "react-router-dom"; // Link navigasi antar halaman
import { useTheme } from "../../context/ThemeContext"; // Ambil tema dari context
import { removeFavorite } from "../../reducer/favoriteReducer"; // Action untuk hapus favorit

const FavoriteView = () => {
  const dispatch = useDispatch(); // Buat dispatch action
  const { films, series } = useSelector((state) => state.favorite); // Ambil films & series dari redux state
  const { theme } = useTheme(); // Ambil tema dari context (dark/light)
  ...
  // Warna fade disesuaikan dengan tema
  const fadeColor = theme === "dark" ? "from-[#1d232a]" : "from-white";

  // Fungsi hapus favorit dengan konfirmasi
  const handleRemove = (id, type) => {
    const typeText = type === "films" ? "movie" : "series"; // Tentukan teks
    const confirmDelete = window.confirm(`Yakin menghapus ${typeText}?`); // Konfirmasi

    if (confirmDelete) {
      dispatch(removeFavorite(id, type)); // Hapus dari redux
    }
  };

  // Render kartu film/series
